...
package-lock.json: _HELP = Lock dependency versions to file
package-lock.json:
	npm i
...
relock:
	rm -rf node_modules/ package-lock.json && $(MAKE) package-lock.json
...
dist/bookmarklet.js: _HELP = Generate bookmarklet
dist/bookmarklet.js: package.json rollup.config.mjs $(wildcard src/* src/*/* src/*/*/* src/*/*/*/*)
	npm run build

dist/bookmarklet.html: _HELP = Generate bookmarklet in bookmarks HTML with favicon
dist/bookmarklet.html: dist/bookmarklet.js $(wildcard icons/*)
	npm run build

.PHONY: build
build: _HELP = Generate bookmarklet (alias)
build: dist/bookmarklet.js dist/bookmarklet.html

.PHONY: pbcopy
pbcopy: _HELP = Generate bookmarklet and copy it to clipboard (macOS)
pbcopy: build
	tee /dev/stderr < dist/bookmarklet.js |pbcopy
